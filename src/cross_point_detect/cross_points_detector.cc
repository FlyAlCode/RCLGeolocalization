...
//             if(point_visited_map.at<uchar>(i,j)==0 && neighbor_img.at<uchar>(i,j)==1){      // record terminal points
//                 keypoint_area.push_back(cv::Rect(j, i, 1, 1));
//                 point_visited_map.at<uchar>(i,j)==1;
//             }
...
void CrossPointDetector::Run(const cv::Mat &input_img, cv::Mat &cross_points_img, std::vector<cv::Rect> &keypoint_area){
    ...
    // debug
//     std::cout<<"CountDisconnectPointsForAllPoints"<<std::endl;
    MergeCrossPoints(input_img, tmp, cross_points_img, keypoint_area);
//     std::cout<<"MergeCrossPoints"<<std::endl;
}
